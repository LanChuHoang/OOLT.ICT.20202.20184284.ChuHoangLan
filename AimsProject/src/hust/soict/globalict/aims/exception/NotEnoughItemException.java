package hust.soict.globalict.aims.exception;

public class NotEnoughItemException extends GetLuckyItemException {

	public NotEnoughItemException() {
		// TODO Auto-generated constructor stub
	}

	public NotEnoughItemException(String message) {
		super(message);
		// TODO Auto-generated constructor stub
	}

	public NotEnoughItemException(Throwable cause) {
		super(cause);
		// TODO Auto-generated constructor stub
	}

	public NotEnoughItemException(String message, Throwable cause) {
		super(message, cause);
		// TODO Auto-generated constructor stub
	}

	public NotEnoughItemException(String message, Throwable cause, boolean enableSuppression,
			boolean writableStackTrace) {
		super(message, cause, enableSuppression, writableStackTrace);
		// TODO Auto-generated constructor stub
	}

}
